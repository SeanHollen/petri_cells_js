...
class GridController {
  ...
  getCellClickEventListener(program, cellUI, pos) {
    ...
    return function(e) {
      ...
      document.getElementById(
        "cell-details-2"
      ).innerHTML = controller.toColoredFormat(program);
  ...
  initState(width, height) {
    ...
    const grid = Array.from({ length: height }, () =>
      Array.from({ length: width }, () => randomData())
    );
  ...
  submitProgram(program, grid) {
    ...
    document.getElementById("cell-details-1").innerText = program.join(",");
    document.getElementById("cell-details-2").innerHTML = this.toColoredFormat(
      program
    );
  ...
  placeProgramsRandomly(grid, program1, program2) {
    const height = grid.length;
    const width = grid[0].length;

    // Place program1
    let x1 = Math.floor(Math.random() * height);
    let y1 = Math.floor(Math.random() * width);
    grid[x1][y1] = program1;

    // Place program2
    let x2, y2;
    do {
      x2 = Math.floor(Math.random() * height);
      y2 = Math.floor(Math.random() * width);
    } while (x2 === x1 && y2 === y1);
    grid[x2][y2] = program2;
...
addEventListener("board-restart-button", () => {
  ...
  const program1 = fromHumanReadableStr(program1Input);
  const program2 = fromHumanReadableStr(program2Input);
